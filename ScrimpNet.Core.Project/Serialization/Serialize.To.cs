...
using ScrimpNet.Serialization.TNet;
...
namespace ScrimpNet
{
    ...
    public static partial class Serialize
    {
        ...
        public class To
        {
            ...
            /// <summary>
            /// Attempt to serialize an object with several different serializers.  Thus caller cannot be guaranteed of the resultant format. Sequence: DataContract, Xml, Soap, Json, ToString, ErrorMessage.
            /// This method is often used for one-way roles (e.g. logging and debugging) since deserialization is not guarantteed
            /// </summary>
            /// <param name="obj">non-null object to serialize</param>
            /// <returns>Serialized version of object or null if <paramref name="obj"/> is null.  Default: DataContract</returns>
            public static string LastChance(object obj)
            {

                try
                {
                    return Serialize.To.DataContract(obj);
                }
                catch
                {
                    try
                    {
                        return Serialize.To.Xml(obj);
                    }
                    catch
                    {
                        try
                        {
                            return Serialize.To.Soap(obj);
                        }
                        catch
                        {
                            try
                            {
                                return Serialize.To.Json(obj);
                            }
                            catch
                            {
                                try
                                ...
                                {
                                    throw new SerializationException(string.Format("Unable to serialize {0} to string", obj.GetType().FullName));
                                }
                            }
                        }
                    }
