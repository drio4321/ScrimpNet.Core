...
namespace ScrimpNet.Diagnostics
{
    ...
    public class TraceProbe : IDisposable
    {
		private class LabelItem
		{
			...
			{
				string s = string.Format("{0:hh:mm:ss} (+{1:0.000} interval {2:0.000} seconds) {3}",
					LabelTime,
        ...
        private void sendLogMessage()
        {
			...
			{				
				_logMessage.ExtendedProperties.Add("TracePoint:"+x.ToString("00"),_traceLabels[x].ToString());
			}
