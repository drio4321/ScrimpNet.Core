    ...
    public partial class CryptoUtils
    {
        ...
        public class Simple
        {
            ...
            public static byte[] Hash(byte[] bytesToHash, byte[] hashSalt = null)
            {
                ...
                if (hashSalt == null)
                {
                    key = HashEncoder.CreateKey(_defaultSimpleHashMode);

                }
                else
                {
                    key = HashEncoder.CreateSaltedKey(_defaultSaltedHashMode, hashSalt);
                }
                using (var encoder = new HashEncoder(key))
                {
                    return encoder.Hash(bytesToHash);
                }
            }
