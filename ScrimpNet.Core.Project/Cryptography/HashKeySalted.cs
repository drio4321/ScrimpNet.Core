...
namespace ScrimpNet.Cryptography
{
    ...
    public class HashKeySalted : CryptoKeyBase
    {
        ...
        {
            var salt = Segments[Crypto.SegmentNames.HashSalt];
			using (KeyedHashAlgorithm hasher = findHashPovider(Properties[Crypto.PropertyNames.HashSaltedMode].Value, salt))
            {
        ...
        public byte[] Salt
        {
            get
            {
                return Segments[Crypto.SegmentNames.HashSalt];
            }
            set
            {
				Segments[Crypto.SegmentNames.HashSalt] = value;
            }
        }
